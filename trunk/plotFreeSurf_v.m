function [V,F] = plotFreeSurf_bbg(surfPath,surfName,hemisphere,alpha)
% PLOTFREESURF plots a freesurfer surface in matlab.
%   plotFreeSurf(surfFile,alpha) will plot the [hemisphere] of surface in 
%       [surfName] with the transparncy value in [alpha] on a black
%       background.
%
...
for hmsphr = hms
    switch hmsphr
        ...
        case 2
            surface = [surfPath 'lh.' surfName];
    end
    
    % read freesurfer surface into matlab.
%     try
        [V,F] = freesurfer_read_surf(surface);
%     catch
%         [V] = freesurfer_read_surf(surface);        
%     end
...
if (magic == QUAD_FILE_MAGIC_NUMBER),
    ...
    vertices1 = fread(fid, Nvertices*3, 'int16') ./ 100 ;
    if (nargout > 1),
...
elseif (magic == TRIANGLE_FILE_MAGIC_NUMBER),
    ...
    vertices1 = fread(fid, Nvertices*3, 'float32');
...
vertices = reshape(vertices1(1:(floor(length(vertices1)./3))*3), 3, floor(length(vertices1)./3))';
fclose(fid);
...
% freesurfer_fread3 - read a 3 byte integer out of a file16
%
